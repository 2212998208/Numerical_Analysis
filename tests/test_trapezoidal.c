...
typedef struct {
    ...
    Trapezoidal_Err expected_err; // 期望的错误码
}TrapTestCase;


static TrapTestCase cases [] = {
    {"f1(x)=x^2", f1, 0.0, 1.0, 10000, 1.0/3.0, 0, TRAP_OK},
    {"f2(x)=sin(x)", f2, 0.0, M_PI, 10000, 2.0, 0, TRAP_OK},
    {"f3(x)=exp(x)", f3, 0.0, 1.0, 10000, M_E - 1.0, 0, TRAP_OK},
    {"f4(x)=1/(1+x^2)", f4, 0.0, 1.0, 10000, M_PI / 4.0, 0, TRAP_OK},
    {"f5(x)=log(x+1)", f5, 0.0, 1.0, 10000, 2 * Log_2 - 1.0, 0, TRAP_OK},
    {"f6(x)=sqrt(x)", f6, 0.0, 1.0, 100000, 2.0 / 3.0, 0, TRAP_OK},
    {"f1(x)=x^2", f1, 1.0, 0.0, 10000, 1.0/3.0, 1, TRAP_ERR_INVAL}, // 错误的积分区间
    {"f1(x)=x^2", f1, 0.0, 1.0, 0, 1.0/3.0, 1, TRAP_ERR_INVAL}, // 错误的最大迭代次数
};

int test_trapezoidal(void) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        // 构造函数
        Trapezoidal trap = NULL;
        Trapezoidal_Err err = TI.trapezoidal_create(tc->f, tc->a, tc->b, tc->max_iter, &trap, tc->name);
        if (err != TRAP_OK) {
            if (tc->expect_error && err == tc->expected_err && err == TRAP_ERR_INVAL) {
        ...
        // 计算积分
        double integral = 0.0;
        err = TI.trapezoidal_integration(&trap, &integral);
        if (err != TRAP_OK) {
            if (tc->expect_error && err == tc->expected_err && err == TRAP_ERR_INVAL) {
                printf("[TEST] %-25s 积分计算参数错误: err=%d PASS\n", tc->name, err);
                passed++;
                TI.trapezoidal_destroy(&trap);
                continue;
            } else {
